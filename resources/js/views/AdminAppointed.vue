        ...
        <div class="gird col-span-8 bg-[#f5f5f5]">
            ...
            <div class="grid gap-4 mx-10 my-10">
                ...
                <div class="grid gap-4">
                    <h1 class="text-3xl text-custom-gray">Appointed Agents!</h1>
                ...
                <div class="grid bg-white w-full rounded-xl p-6 gap-2">
                    <div class="grid h-fit gap-4">
                        <div class="flex gap-4">
                            <h2 class="text-custom-gray float-left text-xl font-bold px-2">Appointed Agents</h2>
                        ...
                        <div class="grid grid-cols-4 w-full h-fit text-sm text-custom-gray font-medium opacity-50 border-b-2 py-4 px-2">
                            ...
                            <p class="grid col">Appointments</p>
                        </div>
                        ...
                        <div class="h-[500px] overflow-y-scroll">
                            <div @click="gotoAgent(agent.id, agent.rocketID)" v-for="(agent, index) in filterView" :key="index" class="grid grid-cols-4 w-full h-fit p-2 rounded-xl text-sm text-custom-gray font-semibold hover:cursor-pointer hover:bg-custom-gray hover:bg-opacity-5">
                                ...
                                <!--Agency Appointments-->
                                <div class="grid col rounded-full bg-custom-gray bg-opacity-10">
                                    <div class="bg-custom-red rounded-full" :style="{width: `${agent.percent}%`}">
                                        <p class="text-white text-center">{{ agent.appointed }} / 8</p>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    created: async function() {
        ...
        .then(response => {
            response.data.forEach(user => {
                if(user.submitted){
                    this.users.push({id: user.id, rocketID: user.rocketMGA_id, agency: user.agency_name, agent: user.agent_name, email: user.email, appointed: 0, percent: 0})
                }
            })
        })
        ...
        for await (const user of this.users){
            ...
            await axios.get('/api/appointed/' + user.rocketID)
            .then(response => {
                if(response.data.aon != null){i += 1}
                if(response.data.beyond != null){i += 1}
                if(response.data.dual != null){i += 1}
                if(response.data.flow != null){i += 1}
                if(response.data.neptune != null){i += 1}
                if(response.data.palomar != null){i += 1}
                if(response.data.sterling != null){i += 1}
                if(response.data.wright != null){i += 1}
            })

            user.appointed = i
            user.percent = user.appointed / 8 * 100
        }
